...
export async function GET(request: NextRequest) {
  try {
    ...
    const approvedSubmissions = user.submissions.filter(s => s.status === "APPROVED" || s.status === "PAID").length
    const pendingSubmissions = user.submissions.filter(s => s.status === "PENDING").length

    // Get recent clips with proper data structure
    const recentClips = user.submissions.map(submission => {
    ...
    const stats = [
      {
        title: "Total Earned",
        value: `$${Number(user.totalEarnings).toFixed(2)}`,
      ...
      {
        title: "Total Views",
        value: Number(user.totalViews).toLocaleString(),
