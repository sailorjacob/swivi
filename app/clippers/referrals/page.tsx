...
function TierCard({ tier }: { tier: typeof referralTiers[0] }) {
  ...
  return (
    <Card className={`bg-neutral-900 border-gray-800 ${tier.current ? 'border-green-500/50' : ''}`}>
      <CardContent className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            <Icon className={`w-6 h-6 ${tier.current ? 'text-green-400' : 'text-gray-400'}`} />
            <div>
              <h3 className={`font-medium ${tier.current ? 'text-green-400' : 'text-white'}`}>
                {tier.name}
              </h3>
...
function RewardCard({ reward }: { reward: typeof rewards[0] }) {
  return (
    <Card className={`bg-neutral-900 border-gray-800 ${reward.status === 'locked' ? 'opacity-60' : ''}`}>
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center space-x-2">
            <Gift className={`w-4 h-4 ${reward.status === 'available' ? 'text-green-400' : 'text-gray-400'}`} />
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-gray-400">Invite friends and earn bonuses for each successful referral.</p>
      </div>
